...
Assume \emph{f} is not 1-1, that means for some a, b \in B, a \neq b, we have f(a) = f(b)
If m, n \in A and m \neq n, we have g(m) = a, and g(n) = b
As \emph{g} is 1-1, we have g(m) \neq g(n), which corresponds with a \neq b
However, as $f\circ g$ = f(g(x)) is 1-1, this means for any x1, x2 \in B, f(g(x1)) \neq f(g(x2))
This contradicts with when x1 = m, and x2 = n, then f(g(m)) = f(g(n))

So the assumption that \emph{f} is not 1-1 is false
Therefore \emph{f} is 1-1

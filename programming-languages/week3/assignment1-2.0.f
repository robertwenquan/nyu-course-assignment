...
c     ==== Initialize string NITEM_STR and INPUT_NUM_STR with '\0'
c     ==== padding is a "WHILE SPACE", ASCII CODE 32 in decimal
      padding = 32
      do i = 1,MAX_STR_BUF
          NITEM_STR(i:i) = char(padding)
          INPUT_NUM_STR(i:i) = char(padding)
      end do

c     ==== Prepare unbuffered read from console
c      CALL SYSTEM ("stty echo -icanon")
      ...
      do i = 1,MAX_STR_BUF
          read(11, err=555, end=555) NITEM_STR(i:i)
          lens = lens + 1
...
c     lens = my_trim_length(NITEM_STR)
...
c     ==== Handle normal case
      do 101 i = 1, NITEM
          ...
          do j = 1,MAX_STR_BUF
              read(11, err=555, end=555) INPUT_NUM_STR(j:j)
              lens = lens + 1

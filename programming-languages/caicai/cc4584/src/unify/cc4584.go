...
func typeTokenizer(LHS string, RHS string){
  ...
  /* FuncType can only Unify with FuncType */
    ...
    LHS = strings.TrimSpace(LHS[1:len(LHS)])
    LHS = strings.TrimSpace(LHS[1:len(LHS)])
    ...
    RHS = strings.TrimSpace(RHS[1:len(RHS)])
    RHS = strings.TrimSpace(RHS[1:len(RHS)])
...
func isValidFuncType(s string) bool{
  ...
  right_part = strings.TrimSpace(right_part[1:len(right_part)])
  ...
  if ! isValidType(right_part[1: len(right_part)]) {
    return false
  }

...
int main(int argc, char *argv[])
{
  ...
  GIT_T * p_return_git = query_git(34);
  MIT_T ** p_return_mit = query_mit(p_return_git);


  GIT_T * p_return_git_2 = query_git(1);
  MIT_T ** p_return_mit_2 = query_mit(p_return_git_2);
  while(*p_return_mit != NULL) {
    printf("%d\n", (*p_return_mit_2)->docid);
    p_return_mit ++;
  }

  MIT_T *** test_inter = (MIT_T ***) malloc(2*sizeof(MIT_T**));

  *test_inter = p_return_mit;
  *(test_inter+1) = p_return_mit_2;

  get_intersection(test_inter);
  printf("GEQ: %d\n", nextGEQ(p_return_mit, 1));
  return 0;
}
